...
```python
# app/models/user.py (示例：SQLAlchemy 2.0 风格注解)
...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(Enum('family', 'volunteer', 'expert', 'admin', 'maintainer'), nullable=False)
    ...
    service_status: Mapped[str] = mapped_column(Enum('online', 'busy', 'offline', 'pending'), default='pending')
    profile_id: Mapped[int] = mapped_column(Integer, ForeignKey('profile.id'))
    # ...

# app/models/profile.py (示例：同样使用注解风格)
class Profile(Base):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    service_hours: Mapped[int] = mapped_column(Integer)
    feedback_score: Mapped[float] = mapped_column(Float)
    public_email: Mapped[str] = mapped_column(String)
