...
- 双文件输出核心规则：
  ...
  - 独立设计报告：包含全部设计细节、验证过程、使用指南等，供追溯与迭代
...
1. 创建文件路径：确保 `docs/prompts-design/` 文件夹存在（如不存在，提示用户确认是否自动创建）
2. 按标准命名规则命名文件：`YYYY-MM-DD_[提示词场景]_提示词最终版.md`
3. 按“独立设计报告格式”组织完整内容（含设计细节、验证过程、适配逻辑等）
4. 验证文件生成：确认文件已成功保存至指定路径，内容完整无遗漏，提示词正文与原目标文件完全一致
...
1. 分情况排查：
   ...
   - 文件名重复：询问用户“文件已存在，是否覆盖？（建议保留历史版本，可修改文件名后缀为_v1、_v2等）”
2. 按用户反馈处理，确保文件成功生成

### 若双文件提示词正文不一致
1. 自动对比两个文件的提示词正文，定位不一致点
2. 以“原目标文件的纯粹提示词”为基准，修正独立报告中的对应内容
3. 重新验证，确保两者提示词正文完全一致
